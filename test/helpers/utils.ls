require! {async, '../bin/utils/database', '../bin/config/config'}
debug = require('debug')('logistic')
...
open-clean-db-and-load-fixtures = !(config, done)->
  ...
  async.each collections, !(collection, next)->
    (err, docs) <-! db.logistic.[collection].insert config[collection], {safe: true}
    next!
  , !->
    done!
...
clean-db-and-load-fixture = !(filepath, fixture-names, done)->
  config = {}
  for name in fixture-names
    config[name] = load-fixture "#{filepath}/fixture/#{name}-in-db"
  open-clean-db-and-load-fixtures config, done
...
module.exports =
  ...
  clean-db-and-load-fixture: clean-db-and-load-fixture
